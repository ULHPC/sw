#!/bin/bash -l
# Time-stamp: <Thu 2021-07-22 12:12 svarrette>
#############################################################################################
# RESIF 3.0 Building Apps Slurm launcher - Testing GPU Builds against prod 2020a software set
#############################################################################################
#
#SBATCH -J RESIF-Test-2020a-GPU
...
VERSION=2020a
if [ -f "${SETTINGS_DIR}/${VERSION}/iris-gpu.sh" ]; then
    source ${SETTINGS_DIR}/${VERSION}/iris-gpu.sh
fi
launcher=${TOP_DIR}/scripts/launcher-test-build-gpu.sh
